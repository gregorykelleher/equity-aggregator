...
async def test_ensure_crumb_returns_cached_crumb_on_fast_path() -> None:
    ...
    result = await manager.ensure_crumb("AAPL", fetch)

    assert result == "cached-crumb"
...
async def test_ensure_crumb_bootstraps_and_returns_crumb() -> None:
    ...
    result = await manager.ensure_crumb("MSFT", fetch)

    assert result == "new-crumb-value"
...
async def test_ensure_crumb_returns_same_crumb_after_bootstrap() -> None:
    ...
    first_result = await manager.ensure_crumb("AAPL", fetch)
    second_result = await manager.ensure_crumb("MSFT", fetch)
