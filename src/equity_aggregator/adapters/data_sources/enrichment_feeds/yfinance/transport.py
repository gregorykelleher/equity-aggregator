...
class HttpTransport:
    """
    Manages HTTP client lifecycle with automatic recovery on connection errors.

    When a request fails due to HTTP/2 protocol errors, the transport resets
    the client and retries. Uses optimistic concurrency: if another task
    already reset the client, the retry is "free" (doesn't consume budget).
